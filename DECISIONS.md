...
- **✅ Renamed the cross-target `Shared/` to `Common/`.** There were two `Shared` folders at different scopes — `StashApp/Shared/` (compiled into the app *and* the extensions) and `StashApp/Stash/Shared/` (app code shared between iOS and macOS). The outer one is now `StashApp/Common/`, leaving a single unambiguous `Stash/Shared/`.
