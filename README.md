...
| `StashApp/` | Native SwiftUI app for **iOS and macOS**, each with a Share Extension | SwiftUI, XcodeGen |
...
- **[XcodeGen](https://github.com/yonaskolb/XcodeGen)** — `StashApp`'s Xcode project is generated, not committed: `brew install xcodegen`
...
`StashApp`'s `.xcodeproj` is generated by XcodeGen from `project.yml`, so generate it first:

```sh
cd StashApp
xcodegen generate        # creates Stash.xcodeproj (re-run after editing project.yml)
open Stash.xcodeproj      # then pick the Stash (iOS) or StashMac scheme and Run
```
