...
- **`StashApp/`** — one multiplatform SwiftUI app (iOS **and** macOS) + an iOS and a macOS Share
  Extension, generated by XcodeGen.
...
**StashApp** (`cd StashApp`) — the `.xcodeproj` is **generated and gitignored**:
```sh
xcodegen generate                    # REQUIRED after editing project.yml or adding/moving/renaming files
...
```
If schemes go missing after an incremental `xcodegen generate`, do a clean regen: `rm -rf Stash.xcodeproj && xcodegen generate`.
...
**StashApp source layout and cross-platform strategy:**
- `StashApp/Shared/` — code compiled into **both** the app and the Share Extensions (KeychainStore,
...
- `StashApp/Stash/` — app-only code, including the single `@main StashApp` whose scene `body` branches
  with `#if os(macOS)` (macOS adds a `Settings` scene + window sizing). `RootView` routes to
  `MainView` (iOS: size-class split / tab bar) or `MacContentView` (macOS: `NavigationSplitView`).
- Keep platform divergence minimal: iOS-only field/keyboard/title modifiers are funneled through
  helpers in `Shared/Support/PlatformModifiers.swift`; whole-view `#if` guards are reserved for genuine
