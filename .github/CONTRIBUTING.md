...
- **`StashApp/`** has **two** multiplatform targets: one SwiftUI app (`Stash`, iOS **and** macOS) and one
  Share Extension (`StashShareExtension`, iOS **and** macOS). `Stash.xcodeproj` is committed and uses
  synchronized folder groups.
- **`Extension/`** is a Manifest v3 WebExtension (Firefox + Chrome/Zen) that saves the current page over
...
- **StashApp source layout** (folders map to synchronized-folder groups → target membership is
  ...
  - Bookmark navigation uses closure-based `NavigationLink { Detail }` (not
    `navigationDestination(for:)`), deliberately, since the list view is reused at several stack
    depths and a declared destination misroutes taps.
